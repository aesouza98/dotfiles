...
local function worker(user_args)
    ...
    spotify_widget:connect_signal("button::press", function(_, _, _, button)
        ...
        elseif (button == 4) then
            awful.spawn("playerctl --player=spotify volume 0.05+", false)  -- scroll up
        elseif (button == 5) then
            awful.spawn("playerctl --player=spotify volume 0.05-", false)  -- scroll down
        elseif (button == 2) then
            awful.spawn("playerctl --player=spotify previous", false)  -- middle click
        elseif (button == 3) then
            awful.spawn("playerctl --player=spotify next", false)  -- right click
        end
